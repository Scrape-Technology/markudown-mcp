...
export const tools = [
    {
        name: "scrape",
        description: "Scrape a single URL and return its content as clean markdown. Ideal for reading articles, documentation, or any public web page.",
        schema: {
    ...
    {
        name: "map",
        description: "Discover all URLs on a website by crawling its sitemap and link graph. Returns a list of URLs found on the site.",
        schema: {
        ...
        async handler(args, client) {
            try {
                const result = await client.map(args.url, {
                    ...
                    blockedWords: args.blockedWords,
    ...
    {
        name: "crawl",
        description: "Recursively crawl a website starting from a URL, following internal links up to a specified depth. Returns markdown content for each page visited.",
        schema: {
    ...
    {
        name: "search",
        description: "Run a Google search and optionally scrape the content of the top results. Returns search result URLs and, when enabled, their full markdown content.",
        schema: {
        ...
        async handler(args, client) {
            try {
                const result = await client.search(args.query, {
                    limit: args.limit,
                    scrapeResults: args.scrapeResults,
                    lang: args.lang,
    ...
    {
        name: "extract",
        description: "Extract structured data from a URL using a natural language prompt. Describe what you want to extract and the API returns it as structured JSON.",
        schema: {
            url: z.string().url().describe("The URL to extract data from"),
            prompt: z
                .string()
                .describe("Natural language description of what to extract, e.g. \"Extract all product names and prices\""),
            timeout: z.number().optional().describe("Timeout in seconds (default: 60)"),
        },
        async handler(args, client) {
            try {
                const result = await client.extract(args.url, args.prompt, { timeout: args.timeout });
                return ok(result);
            }
            catch (e) {
                return err(e);
    ...
    {
        name: "batch_scrape",
        description: "Scrape multiple URLs in parallel. Returns markdown content for all provided URLs. More efficient than calling scrape repeatedly.",
        schema: {
            urls: z
                .array(z.string().url())
                .min(1)
        ...
        async handler(args, client) {
            try {
                const result = await client.batchScrape(args.urls, {
                    timeout: args.timeout,
                });
                return ok(result);
            }
    ...
    {
        name: "screenshot",
        description: "Take a full-page screenshot of a URL. Returns metadata about the screenshot. Note: binary image data cannot be returned as text; the response will contain a reference or base64 note from the API.",
        schema: {
        ...
        async handler(args, client) {
            try {
                const result = await client.screenshot(args.url, {
                    timeout: args.timeout,
                });
                return ok(result);
            }
    ...
    {
        name: "deep_research",
        description: "Scrape multiple URLs and synthesize their content using an LLM to answer a research question. Ideal for comparative analysis or summarizing information across several sources.",
        schema: {
        ...
        async handler(args, client) {
            try {
                const result = await client.deepResearch(args.query, args.urls, {
    ...
    {
        name: "agent",
        description: "Launch an AI agent that autonomously navigates a website to answer a question or complete a task. The agent can click, scroll, and follow links across multiple pages.",
        schema: {
            url: z.string().url().describe("The starting URL for the agent"),
            prompt: z
                .string()
                .describe("The question to answer or task to complete, e.g. \"Find the pricing page and extract all plan names and prices\""),
            maxSteps: z
                .number()
                .optional()
        ...
        async handler(args, client) {
            try {
                const result = await client.agent(args.url, args.prompt, {
                    maxSteps: args.maxSteps,
                    maxPages: args.maxPages,
                    allowNavigation: args.allowNavigation,
    ...
    },
    {
        ...
        description: "Detect whether the content of a URL has changed since it was last checked. Useful for monitoring pages for updates.",
        schema: {
        ...
        async handler(args, client) {
            try {
                const result = await client.changeDetection(args.url, {
                    mainContent: args.mainContent,
                    excludeTags: args.excludeTags,
                    timeout: args.timeout,
                });
